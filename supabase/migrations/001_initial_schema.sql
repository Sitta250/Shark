...
create policy "project_inputs_select" on project_inputs
  for select using (
    exists (select 1 from projects where projects.id = project_inputs.project_id and projects.user_id = auth.uid())
  );
create policy "project_inputs_insert" on project_inputs
  for insert with check (
    exists (select 1 from projects where projects.id = project_inputs.project_id and projects.user_id = auth.uid())
  );
create policy "project_inputs_update" on project_inputs
  for update using (
    exists (select 1 from projects where projects.id = project_inputs.project_id and projects.user_id = auth.uid())
  );
create policy "project_inputs_delete" on project_inputs
  for delete using (
    exists (select 1 from projects where projects.id = project_inputs.project_id and projects.user_id = auth.uid())
  );
